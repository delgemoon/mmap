...
struct produce_request : public request< produce_request >
{
  ...
  using TopicsPartitions = detail::topics_partitions_vector<
...
public:
  ...
  // request.
  void add_value(const bytes& value,
  ...
  // produce request.
  void add_message_set(const message_set& message_set,
                     const std::string& topic_name,
                     int32 partition = 0);
  ...
  void clear_topic(const std::string& topic_name);

  // Clear the message data for the given topic-partition.
  void clear_topic_partition(const std::string& topic_name, int32 partition);

  // Return true, if this request expects a response from the Kafka server
  bool response_expected() const;
