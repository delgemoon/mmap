...
struct offset_fetch_request :
  public request<offset_fetch_request>
{
  friend class request<offset_fetch_request>;

  static int16 ApiKey();

  using topics_partitions_t = detail::topics_partitions_vector<
...
public:
  ...
  void fetch_offset(const std::string& topic_name, int32 partition);
