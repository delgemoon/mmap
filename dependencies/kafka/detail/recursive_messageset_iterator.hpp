...
struct recursive_message_set_iterator : public std::forward_iterator_tag
{
  public:
    ...
    recursive_message_set_iterator &
    operator=(const recursive_message_set_iterator &rhs);

  private:
    using inner_iterator_type = std::shared_ptr< recursive_message_set_iterator >;

    void
    increment();

    bool
    equal(const recursive_message_set_iterator &other) const;
