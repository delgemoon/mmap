  ...
  message_set_(&message_set)
{
  ...
  else if (!outer_iterator_->nested_message_set().empty())
  {
    inner_iterator_.reset(
      new inner_iterator_type::element_type(
        outer_iterator_->nested_message_set()));
  }
}

inline recursive_message_set_iterator::recursive_message_set_iterator(
  const recursive_message_set_iterator& orig) :
  ...
  inner_iterator_()
{
  ...
  {
    inner_iterator_.reset(
      new inner_iterator_type::element_type(*orig.inner_iterator_));
  }
}

inline recursive_message_set_iterator& recursive_message_set_iterator::operator=(
  const recursive_message_set_iterator& rhs)
{
  ...
  {
    inner_iterator_.reset(
      new inner_iterator_type::element_type(*rhs.inner_iterator_));
  }
  else
  {
    inner_iterator_.reset();
...
inline void recursive_message_set_iterator::increment()
{
  if (!message_set_)
  {
    return;
...
inline bool recursive_message_set_iterator::equal(
  const recursive_message_set_iterator& other) const
{
...
inline const message_and_offset& recursive_message_set_iterator::dereference() const
{
  if (!is_inner_done())
  {
...
inline void recursive_message_set_iterator::reset()
{
  message_set_ = NULL;
  inner_iterator_.reset();
}

inline bool recursive_message_set_iterator::is_inner_done() const
{
