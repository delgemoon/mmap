...
inline int32 string_wire_size(const std::string& str)
{
  return static_cast<int32>(sizeof(int16) + str.size());
}

inline int32 bytes_wire_size(const bytes& bytes)
{
...
int32 request_wire_size(const TRequest& request, const std::string& client_id)
{
  return
    sizeof(int16) +  // ApiKey
...
inline void write_int8(int8 value, std::ostream& os)
{
  os.write( reinterpret_cast<char*>(&value), sizeof(int8) );
}

inline void write_int16(int16 value, std::ostream& os)
{
  value = htons(value);
  os.write( reinterpret_cast<char*>(&value), sizeof(int16) );
}

inline void write_int32(int32 value, std::ostream& os)
{
  value = htonl(value);
  os.write( reinterpret_cast<char*>(&value), sizeof(int32) );
}

inline void write_int64(int64 value, std::ostream& os)
{
  value = host_to_be_64(value);
  os.write( reinterpret_cast<char*>(&value), sizeof(int64) );
}

inline void write_string(const std::string& value, std::ostream& os)
{
  write_int16(static_cast<int16>(value.size()), os);
  os.write( value.c_str(), value.size() );
}

inline void write_bytes(const bytes& value, std::ostream& os)
{
  if (!value)
  {
    write_int32(-1, os);
...
template< typename TRequest >
void write_request(const TRequest& request, const std::string& client_id,
                  std::ostream& os)
{
